...
class Dom
  ...
  @vertBtns:( stream, spec, imgDir, w=60, x0=0, y0=0 ) ->
    ...
    for own key, study of spec when UI.isChild(key)
      ...
      iconc = if icon        and  study.iconc then study.iconc  else null
  ...
  @btn:( x, y, w, h, label="", klass="btn-nice", tall=false, icon=null, src=null, mh=null, iconc=null, back="#3B5999" ) ->
    ...
    htm += """<i   class="#{iconsClass} #{icon} fa-lg" style="color:#{iconc}"></i>""" if icon? and iconc?
