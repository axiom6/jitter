...
class Navb
  ...
  id:( spec, ext = '' ) -> Util.htmlId( spec.name, 'Navb', ext )
  ...
  publish:() ->
    for spec in @specs
      if spec.htmlId?
        ...
        @stream.publish( spec.subject, spec.topic, spec.$, eventType ) if spec.topic?
      if spec.items?
        for item in spec.items when item.htmlId?
          item.$ = $('#'+item.htmlId)
          @stream.publish( item.subject, item.topic, item.$, 'click' ) if item.topic?
    return
