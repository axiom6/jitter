  ...
  class Stream {
    ...
    getSubject(name, warn = false) {
      if (this.subjects[name] != null) {
        ...
        if (warn) {
          console.warn('Stream.getSubject() unknown subject so returning new subject for', name);
        }
        this.subjects[name] = new Rx.Subject();
      }
    ...
    concat(name, sources, onComplete) {
      ...
      onError = function(err) {
        return console.log('Stream.concat() error', err);
      };
    ...
    logSubjects() {
      ...
      for (key in ref) {
        obj = ref[key];
        results.push(console.log('Stream.logSubjects', key));
      }
      return results;
    }
